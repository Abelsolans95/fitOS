/**
 * Three-layer exercise resolver (cross-platform — works for web + mobile).
 *
 ...
 * `hidden=true` in Layer C excludes the exercise entirely.
 *
 * The `SupabaseClient` type comes from @supabase/supabase-js — installed in
 * both apps/web and apps/mobile, so the import resolves identically.
 */
...
export async function getResolvedExercises(
  ...
  const [exercisesRes, overridesRes] = await Promise.all([
    supabase
      ...
      .select(
        "id,name,description,video_url,video_thumbnail_url,muscle_groups,secondary_muscles,equipment_needed,category,difficulty,is_global,trainer_id,aliases"
      )
    ...
    supabase
      ...
      .select(
        "id,trainer_id,exercise_id,custom_name,custom_description,custom_notes,custom_video_url,hidden"
      )
  ...
  for (const ov of overrides) overrideMap.set(ov.exercise_id, ov);
