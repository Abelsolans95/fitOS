/**
 * Three-layer food resolver (cross-platform — works for web + mobile).
 *
...
export async function getResolvedFoods(
  ...
  const [foodsRes, overridesRes] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "id,trainer_id,food_id,custom_name,custom_kcal,custom_protein,custom_carbs,custom_fat,custom_fiber,custom_notes,hidden"
      )
