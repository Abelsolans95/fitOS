...
| 144 | CORS | Heurística `origin.endsWith(".vercel.app") && origin.includes("fitos")` acepta `fitos-evil.vercel.app` | Usar regex exacta del proyecto via `VERCEL_PROJECT_PREVIEW_REGEX` env; rechazar por defecto si falta |
| 145 | Test | Tests que importan `next/server` no resuelven `react` en monorepo con React en `apps/web/node_modules` | Symlink `node_modules/react` → `apps/web/node_modules/react` en la raíz del monorepo |
| 146 | Test | Headers `Origin`/`Referer` son "forbidden" en `new Request(..., { headers })` — undici los descarta | En tests, mockear con objeto plano `{ method, headers: { get: name => ... } }` en lugar de `Request` real |
| 147 | Security | `console.error` en API routes no redacta PII — leaks de credenciales en logs de Vercel | Usar `logger.error("[mod] msg", { error })` de `@/lib/logger` — redacta password/token/secret/authorization automáticamente |
| 148 | API | Validación UUID con regex `/i` acepta mayúsculas — Postgres canoniza a minúsculas | Usar `uuidSchema` de `@/lib/validation` (sin flag `/i`) — alineado con el formato de la DB |
| 149 | Arch | Código boilerplate repetido (CSRF+rate-limit+auth+role+body) en cada API route → gotchas por olvido | Usar `handler({ csrf, auth, role, body }, fn)` de `@/lib/api-handler` — aplica el stack de defensas por construcción |
| 150 | DB | Trainer "Hoy" panel queryaba `health_logs.severity` / `reported_at` y `support_tickets.title` — columnas inexistentes (reales: `pain_score` / `created_at` / `subject`) | Verificar nombres de columna en `supabase/migrations/*.sql` antes de queries; los tests con mocks no detectan mismatches de schema |
