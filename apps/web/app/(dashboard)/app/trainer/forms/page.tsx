              ...
              <div className="flex items-center justify-between">
                ...
                {!fields.some((f) => f.type === "section") && (
                  <button
                    type="button"
                    onClick={() => {
                      const template = getOnboardingSectionsTemplate();
                      setFields(fields.length === 0 ? template : [...fields, ...template]);
                      toast.success("Plantilla de secciones cargada");
                    }}
                    className="flex items-center gap-1.5 rounded-lg border border-[#7C3AED]/30 bg-[#7C3AED]/10 px-3 py-1.5 text-[11px] font-semibold text-[#7C3AED] transition-all hover:bg-[#7C3AED]/20"
                  >
