...
export default function TrainerDashboardPage() {
  ...
  const kpiCards = [
    { label: "Clientes activos", value: kpis.activeClients, color: "#00E5FF", icon: "users" },
    { label: "Formularios", value: kpis.pendingForms, color: "#FF9100", icon: "forms" },
    { label: "Rutinas activas", value: kpis.activeRoutines, color: "#7C3AED", icon: "routines" },
    { label: "Menus activos", value: kpis.activeMealPlans, color: "#00C853", icon: "meals" },
  ];
  ...
  const quickActions = [
    { label: "Nueva rutina", href: "/app/trainer/routines", color: "#7C3AED", icon: "M12 4.5v15m7.5-7.5h-15" },
    { label: "Nuevo menu", href: "/app/trainer/nutrition", color: "#00C853", icon: "M12 4.5v15m7.5-7.5h-15" },
    { label: "Ver clientes", href: "/app/trainer/clients", color: "#00E5FF", icon: iconPaths.users },
    { label: "Formularios", href: "/app/trainer/forms", color: "#FF9100", icon: iconPaths.forms },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {kpiCards.map((card, i) => (
          <div
            key={card.label}
            className={`group relative overflow-hidden rounded-2xl border border-white/[0.06] bg-[#12121A] p-5 transition-all duration-300 hover:border-white/[0.12] hover:border-white/[0.15] ${
              i === 0 ? "lg:row-span-1" : ""
            }`}
          >
            ...
            >
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d={iconPaths[card.icon]} />
              </svg>
            </div>

            <p className="text-4xl font-black tracking-tight text-white">{card.value}</p>
            <p className="mt-1 text-xs font-semibold uppercase tracking-wider text-[#5A5A72]">
              {card.label}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3">
          <h2 className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-[#5A5A72]">
            Actividad reciente
          </h2>
