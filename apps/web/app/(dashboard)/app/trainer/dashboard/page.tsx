...
function useTrainerDashboard(user: User | null) {
  ...
  const [kpis, setKpis] = useState<KpiData>({ activeClients: 0, pendingForms: 0, activeRoutines: 0, activeMealPlans: 0 });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setTrainerName(profileRes.data?.full_name || user.user_metadata?.full_name || user.email?.split("@")[0] || "Entrenador");
        setKpis({ activeClients: clientsRes.count ?? 0, pendingForms: formsRes.count ?? 0, activeRoutines: routinesRes.count ?? 0, activeMealPlans: mealsRes.count ?? 0 });
      } catch {
...
export default function TrainerDashboardPage() {
  ...
  if (loading) {
    return (
      <div className="space-y-6 p-1">
        <div className="space-y-2"><Skeleton className="h-3 w-28" /><Skeleton className="h-9 w-56" /></div>
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{[...Array(4)].map((_, i) => <Skeleton key={i} className="h-32" />)}</div>
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-5"><Skeleton className="h-64 lg:col-span-3" /><Skeleton className="h-64 lg:col-span-2" /></div>
      </div>
    );
  }
  ...
  return (
    <>
      <style>{`
        @keyframes dash-fade { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
        @keyframes orb-drift { 0%,100% { transform: translate(0,0) scale(1); } 50% { transform: translate(30px,-20px) scale(1.06); } }
        @keyframes kpi-glow { 0%,100% { opacity: 0.06; } 50% { opacity: 0.12; } }
        @keyframes pulse-dot { 0%,100% { box-shadow: 0 0 0 0 rgba(0,200,83,0.5); } 50% { box-shadow: 0 0 0 5px rgba(0,200,83,0); } }
        @keyframes shimmer-bar { from { background-position: -200% 0; } to { background-position: 200% 0; } }
        .dash-in { animation: dash-fade 0.6s cubic-bezier(0.16,1,0.3,1) both; }
        .d1 { animation-delay: 0.04s; } .d2 { animation-delay: 0.12s; } .d3 { animation-delay: 0.22s; }
        .d4 { animation-delay: 0.34s; } .d5 { animation-delay: 0.46s; } .d6 { animation-delay: 0.58s; }
        .kpi-card { position: relative; overflow: hidden; border-radius: 18px; background: rgba(14,14,24,0.6); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.06); transition: border-color 0.35s ease, transform 0.3s ease, box-shadow 0.35s ease, background 0.3s ease; cursor: pointer; }
        .kpi-card:hover { transform: translateY(-2px); background: rgba(20,20,34,0.7); }
        .kpi-orb { position: absolute; top: -30px; right: -30px; width: 100px; height: 100px; border-radius: 50%; filter: blur(40px); animation: kpi-glow 4s ease-in-out infinite; }
        .action-card { display: flex; align-items: center; gap: 12px; border-radius: 14px; border: 1px solid rgba(255,255,255,0.06); background: rgba(14,14,24,0.5); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); padding: 14px 16px; transition: all 0.25s ease; text-decoration: none; }
        .action-card:hover { background: rgba(255,255,255,0.03); border-color: rgba(255,255,255,0.12); transform: translateX(3px); }
        .action-card-primary { border-color: rgba(124,58,237,0.2); background: rgba(124,58,237,0.08); }
        .action-card-primary:hover { border-color: rgba(124,58,237,0.35); background: rgba(124,58,237,0.15); box-shadow: 0 0 30px -10px rgba(124,58,237,0.3); }
        .shimmer-num { background: linear-gradient(90deg, #fff 0%, rgba(255,255,255,0.6) 50%, #fff 100%); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .live-dot { animation: pulse-dot 2s ease-in-out infinite; }
        .section-label { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3em; color: #5A5A72; margin-bottom: 12px; }
        .empty-state { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px; padding: 48px 24px; color: #5A5A72; }
        @media (prefers-reduced-motion: reduce) { .dash-in, .kpi-orb, .shimmer-num, .live-dot { animation: none !important; } }
      `}</style>
        ...
        <div className="dash-in d1 flex items-start justify-between">
          <div>
            ...
            <h1 className="text-[32px] font-extrabold leading-tight tracking-[-0.03em] text-white">
              {firstName}<span className="text-[#00E5FF]">.</span>
            </h1>
            ...
            <div className="flex flex-col gap-2">
              {quickActions.map((action) => (
                <Link key={action.label} href={action.href} className={`action-card ${action.primary ? "action-card-primary" : ""}`}>
                  <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl" style={{ background: `${action.color}12`, color: action.color }}>
                  ...
                  <span className="flex-shrink-0 text-[#3A3A52]"><IcArrow /></span>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {tips.map((tip, i) => (
              <div key={i} className="relative overflow-hidden rounded-[16px] border border-white/[0.06] bg-[#0E0E18]/60 backdrop-blur-xl p-5 transition-colors hover:bg-white/[0.04]">
                <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: `linear-gradient(90deg, ${tip.accent}, transparent)` }} />
                <div className="pointer-events-none absolute -top-8 -right-8 h-24 w-24 rounded-full opacity-[0.06] blur-[40px]" style={{ background: tip.accent }} />
        ...
        <div className="dash-in d6">
          <div className="relative overflow-hidden rounded-[18px] border border-[#00E5FF]/12 bg-[#00E5FF]/[0.03] p-6">
            ...
            <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Link href="/app/trainer/routines" className="flex-shrink-0 inline-flex items-center gap-2 rounded-xl bg-[#00E5FF] px-5 py-2.5 text-[13px] font-bold text-[#0A0A0F] transition-all hover:bg-[#2BEEFF] hover:shadow-[0_0_24px_rgba(0,229,255,0.35)]">
                Crear rutina <IcPlus />
              </Link>
            </div>
          </div>
