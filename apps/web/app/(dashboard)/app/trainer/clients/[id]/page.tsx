...
type TabKey = "perfil" | "progreso" | "rutina" | "menu" | "formulario" | "comunicacion";
...
function ChatTab({ trainerId, clientId, clientName }: { trainerId: string; clientId: string; clientName: string | null }) {
  ...
  const handleSend = async () => {
    ...
    if (insertErr) {
      ...
      // Keep optimistic visible with error marker — don't rollback
      setMessages((prev) =>
        prev.map((m) =>
          m.id === optimistic.id ? { ...m, id: `err-${Date.now()}` } : m
        )
      );
    } else if (data) {
      setMessages((prev) =>
    ...
    <div className="flex h-[580px] flex-col rounded-2xl border border-white/[0.06] bg-[#0E0E18]/60 backdrop-blur-xl overflow-hidden">
      ...
      <div className="flex items-center gap-3 border-b border-white/[0.06] px-5 py-4">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#7C3AED]/10 text-xs font-bold text-[#7C3AED]">
          {clientName ? clientName.trim().split(/\s+/).slice(0, 2).map((p) => p[0]).join("").toUpperCase() : "??"}
        </div>
        <div>
                  ...
                  <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${
                    ...
                    <div className={`mt-1 flex items-center gap-1 ${isTrainer ? "justify-end" : "justify-start"}`}>
                      ...
                      {isTrainer && !isOptimistic && (
                        <svg className={`h-3 w-3 ${msg.read_at ? "text-[#00E5FF]" : "text-[#5A5A72]"}`} fill="currentColor" viewBox="0 0 24 24">
...
function ClientDetailPageInner() {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-32">
        ...
        <button type="button" onClick={() => router.push(fromChat ? "/app/trainer/chat" : "/app/trainer/clients")} className="text-sm text-[#00E5FF] transition-colors hover:underline">
          {fromChat ? "Volver a chats" : "Volver a clientes"}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <button
        type="button"
        onClick={() => router.push(fromChat ? "/app/trainer/chat" : "/app/trainer/clients")}
      ...
      <div>
        ...
        {activeTab === "comunicacion" && trainerId && (
          <ChatTab trainerId={trainerId} clientId={clientId} clientName={profile.full_name} />
        )}
      </div>
    </div>
  );
