...
export function TabSalud({
  ...
  const fetchLogs = useCallback(async () => {
    ...
    if (error) {
      ...
      console.error("[TabSalud] Error fetching logs:", error);
      return;
    }
  ...
  // Build muscle statuses for the anatomy map from active/recovering logs
  ...
  const handleSubmit = async (data: HealthLogFormData) => {
    ...
    if (existingForMuscle) {
      ...
      if (error) {
        ...
        console.error("[TabSalud] Error updating:", error);
        setSaving(false);
        return;
      }
      ...
      if (error) {
        ...
        console.error("[TabSalud] Error inserting:", error);
        setSaving(false);
        return;
      }
            ...
            <div className="rounded-2xl border border-white/[0.06] bg-[#0E0E18]/60 backdrop-blur-xl p-5">
              ...
              {activeLogs.length === 0 ? (
                <EmptyState
                  ...
                  description="Haz clic en un músculo del mapa para reportar una molestia"
                />
              ) : (
...
function HealthLogCard({
  ...
  const statusCfg = STATUS_CONFIG[log.status] ?? STATUS_CONFIG.active;
  const painColor =
    log.pain_score >= 6 ? "text-[#FF1744]" : "text-[#FF9100]";
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-lg font-black ${painColor}`}>{log.pain_score}</span>
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusCfg.bg} ${statusCfg.text}`}>
              {statusCfg.label}
            </span>
          </div>
          <div className="mt-1 flex items-center gap-2 text-xs text-[#5A5A72]">
            <span>{INCIDENT_LABELS[log.incident_type] ?? log.incident_type}</span>
            <span>·</span>
            <span>{log.reported_by === "coach" ? "Entrenador" : "Cliente"}</span>
            <span>·</span>
