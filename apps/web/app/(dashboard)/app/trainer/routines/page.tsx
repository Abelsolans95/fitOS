...
function parseScheme(scheme: string): {
  ...
  const match = scheme.match(/^(\d+)\s*x\s*(\d+)(?:\s*-\s*(\d+))?\s*(?:\+\s*(\d+)\s*RP)?/i);
  if (!match)
    return { sets: 3, reps_min: 8, reps_max: 12, rest_pause_sets: 0 };

  return {
    sets: Number(match[1]),
    reps_min: Number(match[2]),
    reps_max: match[3] ? Number(match[3]) : Number(match[2]),
    rest_pause_sets: match[4] ? Number(match[4]) : 0,
  };
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="mx-4 w-full max-w-md rounded-2xl border border-white/[0.06] bg-[#12121A] shadow-2xl">
        <div className="flex items-center justify-between border-b border-white/[0.06] px-6 py-4">
          <h3 className="text-sm font-semibold text-white">Añadir ejercicio</h3>
          <button
            type="button"
            onClick={onClose}
  ...
  const addExercise = (dayKey: string, exercise: ExerciseItem) => {
    setTrainingDays((prev) =>
      prev.map((d) => {
        ...
        const defaultScheme = goal === "fuerza" ? "4x3-6" : "3x8-12";
        const parsed = parseScheme(defaultScheme);
        const newEx: RoutineExercise = {
          ...
          reps_max: parsed.reps_max,
          rest_pause_sets: parsed.rest_pause_sets,
          rir: goal === "fuerza" ? 2 : 1,
          target_weight: null,
      ...
      prev.map((d) => {
        ...
        return {
          ...
          exercises: d.exercises.map((e, i) => {
            ...
            // Re-parse scheme if it changed
            if (updates.scheme !== undefined) {
              const parsed = parseScheme(updates.scheme);
              updated.sets = parsed.sets;
              updated.reps_min = parsed.reps_min;
              updated.reps_max = parsed.reps_max;
              updated.rest_pause_sets = parsed.rest_pause_sets;
            }
            return updated;
          }),
        };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="ml-2 text-sm text-[#8B8BA3]">
          {step === 1
            ? "Configuración"
            : step === 2
      ...
      {step === 1 && (
        <div className="space-y-6">
          <h2 className="text-xl font-bold text-white">Configurar mesociclo</h2>
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-[#12121A] p-6 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="h-10 w-full rounded-xl border border-white/[0.08] bg-[#0A0A0F] px-4 text-sm text-white placeholder:text-[#5A5A72] outline-none transition-colors focus:border-[#00E5FF]/50"
              />
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(["fuerza", "hipertrofia"] as const).map((g) => (
                  ...
                  >
                    ...
                    <span className="mt-0.5 block text-xs text-[#5A5A72]">
          ...
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-[#00E5FF] px-6 py-2.5 text-sm font-semibold text-[#0A0A0F] transition-all hover:bg-[#00E5FF]/90 disabled:opacity-40"
            >
              Siguiente
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="space-y-6">
          ...
          {weekDates.length > 0 && (
            <div className="rounded-xl border border-white/[0.06] bg-[#0A0A0F] p-4">
              <p className="mb-2 text-[10px] font-medium uppercase tracking-wider text-[#5A5A72]">
                ...
                {weekDates.map((wd) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-[#8B8BA3]">{wd.date}</span>
                      {dayLabels[wd.day] && (
                        <span className="text-[10px] font-medium uppercase tracking-wider text-[#00E5FF]/60">
                          {dayLabels[wd.day]}
                        </span>
                      )}
                    </div>
                ...
                >
                  ...
                  {isSelected && (
                    <input
                      ...
                      className="mt-3 h-9 w-full rounded-lg border border-white/[0.08] bg-[#0A0A0F] px-3 text-xs font-medium uppercase tracking-wider text-[#00E5FF] placeholder:text-[#5A5A72] placeholder:normal-case placeholder:tracking-normal outline-none transition-colors focus:border-[#00E5FF]/50"
                    />
                  )}
                </div>
              );
          ...
          <div className="flex justify-between">
            <button
              type="button"
              onClick={() => setStep(1)}
              className="text-sm text-[#8B8BA3] transition-colors hover:text-white"
            >
              Atrás
            </button>
            <button
      ...
      {step === 3 && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-white">Ejercicios por día</h2>
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {dateStr && (
                          <span className="text-xs text-[#5A5A72]">{dateStr}</span>
                        )}
                      </div>
                ...
                <div className="px-4 py-3 space-y-3">
                  {day.exercises.length === 0 && (
                    <p className="py-6 text-center text-xs text-[#5A5A72]">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                        ...
                        <div className="col-span-2 sm:col-span-1 space-y-1">
                          <label className="block text-[10px] font-medium uppercase tracking-wider text-[#5A5A72]">
                            Esquema
                          </label>
                          <input
                            type="text"
                            ...
                            onChange={(e) =>
                              updateExercise(day.key, exIndex, { scheme: e.target.value })
                            }
                            placeholder="3x8-12, 1x8-12 + 2RP"
                            className="h-8 w-full rounded-lg border border-white/[0.08] bg-[#12121A] px-3 text-xs text-white placeholder:text-[#5A5A72] outline-none focus:border-[#00E5FF]/50"
                          />
                          {exercise.rest_pause_sets > 0 && (
                            <span className="text-[9px] font-medium text-[#FF9100]">
                              +{exercise.rest_pause_sets} Rest-Pause
                            </span>
                          )}
                        </div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateExercise(day.key, exIndex, {
                                rir: Number(e.target.value) || 0,
                              })
                            }
                            className="h-8 w-full rounded-lg border border-white/[0.08] bg-[#12121A] px-3 text-center text-xs text-white outline-none focus:border-[#00E5FF]/50"
                          />
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateExercise(day.key, exIndex, {
                                target_weight: e.target.value ? Number(e.target.value) : null,
                              })
                            }
                            placeholder="—"
                            className="h-8 w-full rounded-lg border border-white/[0.08] bg-[#12121A] px-3 text-center text-xs text-white placeholder:text-[#5A5A72] outline-none focus:border-[#00E5FF]/50"
                          />
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateExercise(day.key, exIndex, {
                                rest_s: Number(e.target.value) || 0,
                              })
                            }
                            className="h-8 w-full rounded-lg border border-white/[0.08] bg-[#12121A] px-3 text-center text-xs text-white outline-none focus:border-[#00E5FF]/50"
                          />
                  ...
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-white/[0.1] py-2.5 text-xs font-medium text-[#8B8BA3] transition-all hover:border-[#00E5FF]/40 hover:bg-[#00E5FF]/5 hover:text-[#00E5FF]"
                  >
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              onClick={handleSave}
              disabled={saving}
              className="flex items-center justify-center gap-2 rounded-xl bg-[#00E5FF] px-6 py-2.5 text-sm font-semibold text-[#0A0A0F] transition-all hover:bg-[#00E5FF]/90 hover:shadow-[0_0_24px_rgba(0,229,255,0.25)] disabled:opacity-50"
            >
              {saving ? (
                <>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl bg-[#00E5FF] px-5 py-2.5 text-sm font-semibold text-[#0A0A0F] transition-all hover:bg-[#00E5FF]/90 hover:shadow-[0_0_20px_rgba(0,229,255,0.25)]"
        >
          <svg
            className="h-4 w-4"
            fill="none"
      ...
      {routines.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-[#12121A]">
          <div className="hidden border-b border-white/[0.06] px-6 py-3 sm:grid sm:grid-cols-12 sm:gap-4">
            <div className="col-span-3 text-xs font-medium uppercase tracking-wider text-[#5A5A72]">
              Título
            </div>
            <div className="col-span-2 text-xs font-medium uppercase tracking-wider text-[#5A5A72]">
              Cliente
            </div>
            <div className="col-span-2 text-xs font-medium uppercase tracking-wider text-[#5A5A72]">
              Objetivo
            </div>
            <div className="col-span-1 text-xs font-medium uppercase tracking-wider text-[#5A5A72]">
              Meses
            </div>
            <div className="col-span-1 text-xs font-medium uppercase tracking-wider text-[#5A5A72]">
              Estado
