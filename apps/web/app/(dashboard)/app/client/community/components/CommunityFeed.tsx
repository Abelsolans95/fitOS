  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {posts.map((post) => {
        ...
        return (
          <div key={post.id} className={`rounded-2xl border bg-[#12121A] p-5 transition-all ${post.is_pinned ? "border-[#7C3AED]/30 ring-1 ring-[#7C3AED]/10" : "border-white/10"}`}>
            {post.is_pinned && (
            ...
            <div className="mt-4 flex items-center gap-4 border-t border-white/[0.04] pt-3">
              ...
              <button onClick={() => onToggleExpand(post.id)} className={`flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs transition-all ${isExp ? "text-[#00E5FF]" : "text-[#5A5A72] hover:text-[#00E5FF]"}`}>
              ...
              <div className="mt-3 space-y-3 border-t border-white/[0.04] pt-3">
                ...
                {!isLoadingComments && postComments.map((c) => (
                  <ClientCommentItem
                    ...
                    onSetReplyingTo={(cId) => onSetReplyingTo(post.id, cId)}
                    onSetReplyText={onSetReplyText}
                    onAddComment={onAddComment}
                  />
                ))}
                <div className="flex gap-2">
                  <input type="text" value={newComment[post.id] ?? ""} onChange={(e) => onSetNewComment(post.id, e.target.value)} placeholder="Escribe un comentario..." className="flex-1 rounded-lg border border-white/10 bg-[#0A0A0F] px-3 py-2 text-xs text-white placeholder-[#5A5A72] focus:border-[#00E5FF]/30 focus:outline-none" onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); onAddComment(post.id); } }} />
                  <button onClick={() => onAddComment(post.id)} disabled={!(newComment[post.id]?.trim())} className="rounded-lg bg-[#00E5FF]/10 px-3 py-2 text-xs font-medium text-[#00E5FF] transition-all hover:bg-[#00E5FF]/20 disabled:opacity-40">Enviar</button>
                </div>
              </div>
            )}
          </div>
