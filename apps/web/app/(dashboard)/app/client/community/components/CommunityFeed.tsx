            ...
            {isExp && (
              <div className="mt-3 space-y-3 border-t border-white/[0.04] pt-3">
                ...
                <div className="flex gap-2">
                  <input type="text" value={newComment[post.id] ?? ""} onChange={(e) => onSetNewComment(post.id, e.target.value)} placeholder="Escribe un comentario..." className="flex-1 rounded-lg border border-white/[0.06] bg-[#0A0A0F] px-3 py-2 text-xs text-white placeholder-[#5A5A72] focus:border-[#00E5FF]/30 focus:outline-none" onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); onAddComment(post.id); } }} />
