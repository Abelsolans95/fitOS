...
export default function BuffetAnalysis({
  userId,
  remainingMacros,
  alreadyEatenToday,
  selectedMealType,
  ...
  const recordVideo = useCallback(async () => {
    ...
    if (status !== "granted") {
      Alert.alert("Permiso necesario", "Necesitamos acceso a la camara para grabar el buffet.");
      return;
    }
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      setIdentifiedItems([]);
      setContext(null);
      // After picking video, take a photo as the "best frame" approach for mobile
      // Since video frame extraction in RN is complex, we take a separate photo
      promptForFrameCapture();
    }
  }, []);
  ...
  const promptForFrameCapture = useCallback(() => {
    Alert.alert(
      "Capturar frame",
      "Para el mejor analisis, toma una foto del momento mas representativo del buffet.",
      [
        { text: "Tomar foto", onPress: captureFrame },
  ...
  const captureFrame = useCallback(async () => {
    setExtracting(true);
    try {
      // On mobile, the most reliable way to get a "frame" is to take a photo
      const result = await ImagePicker.launchCameraAsync({
        mediaTypes: ["images"],
        quality: 0.7,
        base64: true,
      ...
      if (!result.canceled && result.assets[0]) {
        setFrameBase64(result.assets[0].base64 ?? null);
  ...
  // Alternative: directly take a photo of the buffet (simpler flow)
  const takeBuffetPhoto = useCallback(async () => {
    const { status } = await ImagePicker.requestCameraPermissionsAsync();
    if (status !== "granted") {
  ...
  const pickBuffetImage = useCallback(async () => {
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ["images"],
      quality: 0.7,
      base64: true,
  ...
  const analyzeBuffet = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        Alert.alert("Error", "No se pudo analizar el buffet. Intentalo de nuevo.");
        console.error("[BuffetAnalysis] suggest-meal-from-image:", error);
        return;
      }
      ...
      Alert.alert("Error", "Error inesperado al analizar el buffet.");
    } finally {
      setAnalyzing(false);
    }
  }, [frameBase64, remainingMacros, alreadyEatenToday]);
  ...
  const saveSuggestion = useCallback(async (suggestion: MealSuggestion) => {
    ...
    try {
      ...
      if (insertError) {
        ...
        console.error("[BuffetAnalysis] food_log insert:", insertError);
        return;
      }
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Frame preview or capture area */}
      {frameUri ? (
        <View style={styles.imageContainer}>
          <Image source={{ uri: frameUri }} style={styles.image} resizeMode="cover" />
          <View style={styles.frameBadge}>
            <Text style={styles.frameBadgeText}>BUFFET</Text>
          </View>
        ...
        <View style={styles.captureRow}>
          <TouchableOpacity style={styles.captureMain} onPress={takeBuffetPhoto} activeOpacity={0.85}>
            <LinearGradient colors={["#FF9100", "#FF1744"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.captureGradient}>
              ...
              <Text style={styles.captureMainText}>Foto buffet</Text>
            </LinearGradient>
          </TouchableOpacity>
          <TouchableOpacity style={styles.captureSecondary} onPress={pickBuffetImage} activeOpacity={0.8}>
            <Svg width={20} height={20} viewBox="0 0 24 24" fill="none">
              <Path
      ...
      {frameBase64 && suggestions.length === 0 && (
        <TouchableOpacity
          ...
          onPress={analyzeBuffet}
          disabled={analyzing}
          activeOpacity={0.85}
        >
          <LinearGradient colors={["#FF9100", "#FF1744"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.analyzeGradient}>
            {analyzing ? (
              <ActivityIndicator color={colors.white} />
            ) : (
              <Text style={styles.analyzeText}>Recomendar plato optimo</Text>
            )}
          </LinearGradient>
        </TouchableOpacity>
      )}
      ...
      {identifiedItems.length > 0 && (
        <View style={styles.identifiedCard}>
          <Text style={styles.sectionLabel}>OPCIONES DEL BUFFET</Text>
...
const styles = StyleSheet.create({
  ...
  frameBadgeText: { fontSize: 9, fontFamily: fonts.bold, color: colors.orange, letterSpacing: 1 },
  clearButton: {
    position: "absolute",
    top: 12,
    right: 12,
  ...
  analyzeButton: { borderRadius: radius.lg, overflow: "hidden", ...shadows.glow(colors.orange) },
  ...
  chip: { backgroundColor: colors.orangeDim, borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4 },
  chipText: { fontSize: 11, fontFamily: fonts.medium, color: colors.orange },
